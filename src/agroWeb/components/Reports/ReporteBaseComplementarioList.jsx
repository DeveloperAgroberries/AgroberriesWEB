...
import { getWorkers } from '../../../store/slices/workers';
...
export const ReporteBaseComplementarioList = ({ isSearchTriggered }) => {
	...
	const { workers = [], isLoading: isLoadingWOrkers, errorMessage: errorWorkers } = useSelector((state) => state.workers);
			...
			{updatedDataFinal.length > 0 ? (
				updatedDataFinal.map((report, index) => (
					<tr key={index + 1}>
						...
						<td>{report.vNombreTra}</td>
						<td>{report.cNoimssTra}</td>
						<td>{report.cRfcTra}</td>
						<td>{report.cCurpTra}</td>
